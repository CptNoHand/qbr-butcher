    ...
    for k = 1, #Config.shops do 
        ...
        if distance < 3.0 then
            ...
            if holding ~= false then
                for i, row in pairs(Config.Animal) do
                    if type == 28 then
                        if model == Config.Animal[i]["model"] then
                            ...
                            if deleted then
                                TriggerServerEvent("qbr-butcher:server:giveitem", Config.Animal[i]["item"], 1)
                                TriggerServerEvent("qbr-butcher:server:reward", reward, level)
                                QBCore.Functions.Notify("You earned $" .. reward .. ", " .. level .. " xp and " .. Config.Animal[i]["item"] .." Meat!")
                            else
                                QBCore.Functions.Notify("Error, please try to relog!", "error")
                            end
                    ...
                    if quality ~= false then
                        if quality == Config.Animal[i]["poor"] then
                            ...
                            if deleted then
                                TriggerServerEvent("qbr-butcher:server:reward", reward, level)
                                QBCore.Functions.Notify("You earned $" .. reward .. " and " .. level .. " xp!")
                            else
                                QBCore.Functions.Notify("Error, please try to relog!", "error")
                            end
                ...
                QBCore.Functions.Notify("Not Holding Anything", "success")
            end
        end
    end
end
